...
export const cekCommand: SlashCommand = {
  data: new SlashCommandBuilder()
    .setName('çek')
    .setDescription('Bir üyeyi bulunduğun ses odasına çekmek için istek gönderir.')
    .addUserOption((opt) => opt.setName('üye').setDescription('Yanına çekmek istediğin üye').setRequired(true)),
  cooldown: 5,
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (targetUser.id === interaction.user.id) {
      await interaction.reply({
        embeds: [createErrorEmbed('Geçersiz İstek', 'Kendini çekme isteği gönderemezsin.')],
        ephemeral: true,
      });
      return;
    }
    ...
    if (targetUser.bot) {
      await interaction.reply({
        embeds: [createErrorEmbed('Geçersiz İstek', 'Botları ses odasına çekemezsin.')],
        ephemeral: true,
      });
      return;
    }
    ...
    const requesterVoice = member.voice.channel as VoiceBasedChannel;

    // Eğer çağıranın zaten "MoveMembers" yetkisi varsa doğrudan çekilsin
    if (member.permissions.has(PermissionFlagsBits.MoveMembers)) {
      await targetMember.voice.setChannel(requesterVoice).catch(() => {});
      await interaction.reply({
        embeds: [
          createSuccessEmbed(
            'Üye Çekildi',
            `👑 Yetkili olduğun için <@${targetUser.id}> doğrudan senin odana (**${requesterVoice.name}**) taşındı!`
          ),
        ],
      });
      return;
    ...
    const acceptBtn = new ButtonBuilder()
      .setCustomId(`cek_accept_${interaction.id}`)
      .setLabel('Kabul Et')
      .setEmoji('✅')
      .setStyle(ButtonStyle.Success);

    const declineBtn = new ButtonBuilder()
      .setCustomId(`cek_decline_${interaction.id}`)
      .setLabel('Reddet')
      .setEmoji('❌')
    ...
    const embed = createEmbed({
      title: '📞 Ses Odasına Çekme İsteği',
      description: `👋 <@${targetUser.id}>, <@${interaction.user.id}> seni kendi bulunduğu **${requesterVoice.name}** ses odasına çekmek istiyor.\n\nKabul ediyor musun?`,
    ...
    collector.on('collect', async (i) => {
      ...
      if (i.customId === `cek_accept_${interaction.id}`) {
        // Her iki tarafın hala seste olup olmadığını kontrol et
        ...
        try {
          await refreshedTarget.voice.setChannel(refreshedMember.voice.channel);
          await i.update({
            content: null,
            embeds: [
              createSuccessEmbed(
                'İstek Kabul Edildi!',
                `✅ <@${targetUser.id}> odaya çekilmeyi kabul etti ve başarıyla **${refreshedMember.voice.channel.name}** odasına taşındı. 🎧`
              ),
            ],
            components: [],
          });
      ...
      } else if (i.customId === `cek_decline_${interaction.id}`) {
        await i.update({
          content: null,
          embeds: [
            createEmbed({
              title: '❌ İstek Reddedildi',
              description: `<@${targetUser.id}>, ses odasına çekilme isteğini reddetti.`,
        ...
        await interaction.editReply({
          embeds: [
            createEmbed({
              ...
              description: `<@${targetUser.id}> 60 saniye içinde yanıt vermediği için ses çekme isteği iptal edildi.`,
